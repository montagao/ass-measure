...
Napi::Value MeasureDimensions(const Napi::CallbackInfo &info) {
  ...
  SubtitleRenderInfo result =
      measure_subtitle_dimensions(filePath.c_str(), videoWidth, videoHeight);

  // Check if measurement was successful
  if (result.count == 0 || result.lines == NULL) {
    Napi::Error::New(env, "Failed to measure subtitle dimensions")
        .ThrowAsJavaScriptException();
    return env.Null();
  }
...
Napi::Object Init(Napi::Env env, Napi::Object exports) {
  exports.Set(Napi::String::New(env, "measureDimensions"),
